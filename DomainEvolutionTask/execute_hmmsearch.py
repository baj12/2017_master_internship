...
for protein in bar(protein_folder) :
    if not os.path.isdir('{}_oskar_search'.format(protein)):
    	os.mkdir('{}_oskar_search'.format(protein))

    path = os.path.join(proteinPath, protein)
    if os.path.isfile(path) :
        output = re.findall(r'^tsa.([A-Z]{4}.[0-9]{1})',protein)[0]
        os.system('hmmsearch --tblout ./{}_oskar_search/{}_osk_search.out {} {}'.format(protein,output,oskModel,protein))
        os.system('hmmsearch --tblout ./{}_oskar_search/{}_lotus_search.out {} {}'.format(protein,output,lotusModel,protein))
